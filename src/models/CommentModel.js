...
export class CommentModel extends BaseModel {
  ...
  async findById(id) {
    const rows = await this.query(`SELECT * FROM comments WHERE id = :id`, { id });
    return rows[0] || null;
  }
  async listByPost({ post_id, include_inactive = false }) {
    const where = include_inactive ? `post_id = :post_id` : `post_id = :post_id AND status='active'`;
    return this.query(`SELECT * FROM comments WHERE ${where} ORDER BY publish_date ASC`, { post_id });
  }
  async updateById(id, data) {
    ...
    await this.query(`UPDATE comments SET ${fields.join(', ')} WHERE id = :id`, params);
