...
export const PostController = {
  async list(req, res) {
    const { page = 1, limit = 10, sortBy = 'likes', category_id, status, date_from, date_to } = req.query;
    const viewer_id = req.user?.id;
    const offset = (Math.max(1, +page) - 1) * +limit;
    const posts = await Posts.list({ limit, offset, sortBy, category_id, status, date_from, date_to, viewer_id });
    res.json(posts);
  },
  async getById(req, res) {
    ...
    if (post.status === 'inactive' && (!req.user || (req.user.role !== 'admin' && req.user.id !== post.author_id))) {
  ...
  async create(req, res) {
    ...
    const post = await Posts.create({ author_id: req.user.id, title, content, status: 'active' });
    // attach categories
    if (Array.isArray(categories) && categories.length) {
      const values = categories.map(cid => `(${post.id}, ${+cid})`).join(',');
      await pool.query(`INSERT IGNORE INTO post_categories (post_id, category_id) VALUES ${values}`);
    }
  ...
  async update(req, res) {
    ...
    if (req.user.role === 'admin') {
      // admin: can change category bindings and status only
      const data = {};
      ...
      if (Array.isArray(req.body.categories)) {
        ...
        if (req.body.categories.length) {
          const values = req.body.categories.map(cid => `(${id}, ${+cid})`).join(',');
          await pool.query(`INSERT IGNORE INTO post_categories (post_id, category_id) VALUES ${values}`);
        }
      }
      return res.json(await Posts.findById(id));
    } else {
      // author: can edit title, content, categories
      if (post.author_id !== req.user.id) return res.status(403).json({ error: 'Forbidden' });
      const allowed = {};
      if ('title' in req.body) allowed.title = req.body.title;
      if ('content' in req.body) allowed.content = req.body.content;
      if (Object.keys(allowed).length) await Posts.updateById(id, allowed);
      if (Array.isArray(req.body.categories)) {
        await pool.query(`DELETE FROM post_categories WHERE post_id = :id`, { id });
        if (req.body.categories.length) {
          const values = req.body.categories.map(cid => `(${id}, ${+cid})`).join(',');
  ...
  async listComments(req, res) {
    ...
    const comments = await Comments.listByPost({ post_id: id, include_inactive });
    res.json(comments);
  },
  ...
  async likeCreate(req, res) {
    ...
    if (!post) return res.status(404).json({ error: 'Post not found' });
    try {
      await Likes.addForPost({ author_id: req.user.id, post_id: id, type });
    } catch (e) {
      return res.status(409).json({ error: 'Already liked' });
    }
    // update rating of post author
...
async function updateUserRating(user_id) {
  // rating = sum of likes on all user's posts and comments
